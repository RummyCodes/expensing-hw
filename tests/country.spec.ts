...
test("should be able to choose country", async ({ page }) => {
  ...
  await signUpPage.acceptTerms.click({ position: { x: 0, y: 0 } }); //I tried not to use  .click({ force: true });  because this can cause issues and we can miuss bug, when user can't interact with element 
  ...
  await signUpPage.countryList.locator('li', { hasText: searchCountry }).click({ force: true });    //I failed


  //what to improve: 
  //4. Добавить find issues к письму и от чего в целом была боль 
  //5. написать отдельный автотес на api который будет делать тест call к  https://circula-qa-challenge.vercel.app/api/countries?locale=en&intent=REGISTRATION и проверять что страна sweden есть в списке 
})